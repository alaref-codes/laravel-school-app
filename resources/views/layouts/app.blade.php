        ...
        <ul class="navbar-nav ml-auto">
                <li class="nav-item dropdown">
                    ...
                    aria-haspopup="true" aria-expanded="false"><span class="flag-icon flag-icon-us">
                        </span> {{ session('lang') ? session('lang') : config('app.locale') }}</a>
                        {{-- == 'en' ? 'العربية' : 'English'  --}}
                    <div class="dropdown-menu" aria-labelledby="dropdown09">
                        <a class="dropdown-item" href="{{ url('locale/en') }}"><span class="flag-icon flag-icon-en"> </span>English</a>
                        <a class="dropdown-item" href="{{ url('locale/ar') }}"><span class="flag-icon flag-icon-ar"> </span>العربية</a>
                    </div>
                </li>
                <li class="nav-item dropdown user-menu">
                    ...
                    <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                        ...
                        <li class="user-header bg-primary">
                            <img src="/storage/{{config('constants.folders.users')}}/{{auth()->user()->image ? auth()->user()->image : 'default-user-image.jpg'}}"
                                class="img-circle elevation-2" alt="User Image">
                            <p>
                                {{ Auth::user()->name }}
                                <small>@lang('static.member_since') {{ Auth::user()->created_at->format('M. Y') }}</small>
                            </p>
                        </li>
                        <!-- Menu Footer-->
                        <li class="user-footer">
                            {{-- <a href="#" class="btn btn-default btn-flat">@lang('static.profile')</a> --}}
                            <a href="#" class="btn btn-default btn-flat float-right"
                                onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
                                @lang('static.sign_out')
                            </a>
